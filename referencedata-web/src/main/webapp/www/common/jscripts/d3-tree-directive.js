 ...
 * @created: 04/03/15
 * @update Alexandre BOURGNEUF : Add EBX features
 */
...
function AngularD3multiParentDirective() {
    ...
    function link(scope, element, attr) {
        ...
        //Put some space between nodes vertically
        ...
        var node = svg.selectAll("g.node").data(nodes).enter().append("svg:g").attr("transform", function (d) {
            ...
            if (!!d.description) {
                ...
                //Update from 8 --> 15
                tooltip.html(d.description).style("left", d3.event.pageX - 15 + "px").style("top", d3.event.pageY + 15 + "px");
            }
...
//    });
        ...
        node.append("svg:circle").attr("r", 10).attr("fill", function (d) {
			var currentColor = "#ffffff";
			 if (!!d.colour) {
				 currentColor = d.colour;
			 }
            return currentColor;
        ...
        //CHange 8 --> 15 and .attr("dy", 3) --> .attr("dy", 5)
        node.append("svg:text").attr("dx", function (d) {
            return d.children ? -15 : 15;
        }).attr("dy", 5).classed("text", true).attr("text-anchor", function (d) {
